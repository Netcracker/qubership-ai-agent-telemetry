...
assert_query distinct_events 7 "$selector | stats count_uniq(event.id) distinct_events"
assert_query raw_id_events 8 "$selector | stats count(event.id) raw_id_events"
