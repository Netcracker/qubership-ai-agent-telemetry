...
setup_component_fixture() {
  ...
  unset CYBER_FERRET_PASSWORD QDI_FAIL_APM_COMMAND
  mkdir -p "$HOME" "$FIXTURE_ROOT/bin"
  : > "$QDI_TEST_LOG"
...
teardown_component_fixture() {
  rm -rf "$FIXTURE_ROOT"
  unset FIXTURE_ROOT HOME XDG_DATA_HOME QDI_TEST_LOG QDI_GIT_CONFIG QDI_APM_STATE
...
test_declined_prerequisite_installation_stops_bootstrap() {
  ...
  assert_contains "$output" "Installation stopped"
}
...
test_default_install_runs_every_component() {
  ...
  assert_log_contains "telemetry-installer --hooks=claude,codex,cursor"
...
test_selection_and_harnesses_are_forwarded() {
  ...
  assert_log_contains "telemetry-installer --hooks=codex"
...
test_force_update_refreshes_selected_components() {
  ...
  assert_log_contains "telemetry-installer --hooks=claude --force"
